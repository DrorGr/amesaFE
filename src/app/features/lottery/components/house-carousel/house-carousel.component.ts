...
import { HeartAnimationService } from '@core/services/heart-animation.service';
import { ProductService } from '../../../payment/services/product.service';
import { IdentityVerificationService } from '../../../user/services/identity-verification.service';
import { VerificationGateComponent } from '../../../user/components/verification-gate/verification-gate.component';
import { PaymentPanelService } from '../../../payment/services/payment-panel.service';
import { PaymentMethodPreferenceService } from '@features/payment/services/payment-method-preference.service';
                    ...
                    <div class="flex space-x-1 mt-1">
                      @for (image of house.images; track $index) {
                        <button 
                          ...
                          type="button"
                          class="w-3 h-3 sm:w-2 sm:h-2 rounded-full transition-all hover:scale-125 cursor-pointer focus:outline-none p-2 sm:p-1 min-w-[32px] min-h-[32px] sm:min-w-0 sm:min-h-0 flex items-center justify-center"
                          ...
                          <button 
                            ...
                            <span class="relative z-10 flex flex-col sm:flex-row items-center gap-1 sm:gap-2 break-words">
                              @if (isPurchasing(house.id)) {
                                ...
                                <span>{{ translate('house.buyTicket') }}</span>
                                <span class="font-bold">{{ formatPrice(house.ticketPrice) }}</span>
                              }
                            </span>
                          </button>
                        }
