...
import { TopbarComponent } from '../features/shared/components/topbar/topbar.component';
import { TranslationLoaderComponent } from '../features/shared/components/translation-loader/translation-loader.component';
import { ChatbotComponent } from '../features/shared/components/chatbot/chatbot.component';
...
import { CookieConsentComponent } from '../features/shared/components/cookie-consent/cookie-consent.component';
import { SkipLinksComponent } from '../features/shared/components/skip-links/skip-links.component';
import { ResponsivePaymentPanelComponent } from '../features/payment/components/responsive-payment-panel/responsive-payment-panel.component';
import { TranslationService } from '../core/services/translation.service';
import { ToastService } from '../core/services/toast.service';
import { CookieConsentService } from '../core/services/cookie-consent.service';
import { PaymentPanelService } from '../features/payment/services/payment-panel.service';
...
@Component({
  ...
  template: `
    <div class="min-h-screen bg-gradient-to-br from-gray-50 to-blue-50 dark:from-gray-900 dark:to-gray-800 transition-all duration-500 ease-in-out">
      ...
      <footer id="footer" role="contentinfo" class="bg-gradient-to-r from-gray-900 to-gray-800 dark:from-gray-950 dark:to-gray-900 text-white py-1 sm:py-5 md:py-8">
        <div class="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8">
          <!-- Logo, Description and Social Media -->
          <div class="mb-0.5 sm:mb-3 md:mb-4 flex flex-col md:flex-row md:items-center md:justify-between gap-0.25 sm:gap-4 md:gap-6 mobile-footer-header">
            <div class="flex items-center gap-0.25 sm:gap-4 md:gap-6 mobile-footer-logo-section">
              <img 
                src="assets/AmesaNoBG.png" 
                alt="Amesa" 
                class="h-4 sm:h-20 md:h-36 w-auto mobile-footer-logo">
              <p class="text-gray-300 text-[8px] sm:text-base md:text-2xl leading-tight sm:leading-relaxed mobile-footer-description">
                {{ translate('footer.description') }}
              </p>
            </div>
            <!-- Social Media Links -->
            <div class="flex items-center space-x-0.5 sm:space-x-3 md:space-x-4 mobile-footer-social">
          ...
          <!-- Footer Links Grid -->
          <div class="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-5 gap-0.5 sm:gap-3 md:gap-6 mb-0.5 sm:mb-4 md:mb-6 mobile-footer-links">
            <!-- Community -->
            <div>
              <h4 class="font-semibold mb-0.5 sm:mb-2 md:mb-3 text-white text-[12px] sm:text-xs md:text-sm lg:text-base mobile-footer-section-heading">{{ translate('footer.community') }}</h4>
              <ul class="space-y-0 sm:space-y-1 md:space-y-2 text-gray-300 text-[12px] sm:text-xs md:text-sm mobile-footer-link-text">
            ...
            <!-- Support -->
            <div>
              <h4 class="font-semibold mb-0.5 sm:mb-2 md:mb-3 text-white text-[12px] sm:text-xs md:text-sm lg:text-base mobile-footer-section-heading">{{ translate('footer.support') }}</h4>
              <ul class="space-y-0 sm:space-y-1 md:space-y-2 text-gray-300 text-[12px] sm:text-xs md:text-sm mobile-footer-link-text">
            ...
            <!-- Legal -->
            <div>
              <h4 class="font-semibold mb-0.5 sm:mb-2 md:mb-3 text-white text-[12px] sm:text-xs md:text-sm lg:text-base mobile-footer-section-heading">{{ translate('footer.legal') }}</h4>
              <ul class="space-y-0 sm:space-y-1 md:space-y-2 text-gray-300 text-[12px] sm:text-xs md:text-sm mobile-footer-link-text">
                ...
                <li><a href="#" [attr.aria-label]="translate('footer.gdprInfo')" class="hover:text-blue-400 transition-colors duration-200 hover:translate-x-1 transform inline-block focus:outline-none rounded">{{ translate('footer.gdprInfo') }}</a></li>
                <li><button (click)="openCookiePreferences()" (keydown.enter)="openCookiePreferences()" (keydown.space)="openCookiePreferences(); $event.preventDefault()" [attr.aria-label]="translate('footer.cookiePreferences')" class="hover:text-blue-400 transition-colors duration-200 hover:translate-x-1 transform inline-block text-left focus:outline-none rounded">{{ translate('footer.cookiePreferences') }}</button></li>
                <li><a href="#" [attr.aria-label]="translate('footer.news')" class="hover:text-blue-400 transition-colors duration-200 hover:translate-x-1 transform inline-block focus:outline-none rounded">{{ translate('footer.news') }}</a></li>
              </ul>
            </div>
            
                <!-- Legal Partners -->
                <div>
                  <h4 class="font-semibold mb-0.5 sm:mb-2 md:mb-3 text-white text-[12px] sm:text-xs md:text-sm lg:text-base mobile-footer-section-heading">{{ translate('footer.legalPartners') }}</h4>
                  <div class="space-y-0.5 sm:space-y-2 md:space-y-3">
                    <div>
                      <div class="text-gray-400 text-[12px] sm:text-xs mb-0 sm:mb-1 mobile-footer-link-text">{{ translate('partners.legalPartner') }}</div>
                      <a href="https://ziebapartners.com" target="_blank" rel="noopener noreferrer" class="text-gray-300 hover:text-blue-400 transition-colors duration-200 hover:translate-x-1 transform inline-block text-[12px] sm:text-xs md:text-sm mobile-footer-link-text">
                        Zeiba & Partners
                      </a>
                    </div>
                    <div>
                      <div class="text-gray-400 text-[12px] sm:text-xs mb-0 sm:mb-1 mobile-footer-link-text">{{ translate('partners.accountingPartner') }}</div>
                      <a href="https://pik.tax/kontakt" target="_blank" rel="noopener noreferrer" class="text-gray-300 hover:text-blue-400 transition-colors duration-200 hover:translate-x-1 transform block sm:inline-block text-[12px] sm:text-xs md:text-sm lg:text-base break-words whitespace-normal mobile-footer-link-text">
            ...
            <!-- App Download -->
            <div>
              <h4 class="font-semibold mb-0.5 sm:mb-2 md:mb-3 text-white text-[12px] sm:text-xs md:text-sm lg:text-base mobile-footer-section-heading">{{ translate('footer.comingSoon') }}</h4>
              <div class="flex flex-col space-y-0.5 sm:space-y-1.5 md:space-y-2">
                <div class="bg-gray-600 rounded-lg px-1.5 sm:px-3 md:px-4 py-1 sm:py-2 md:py-3 flex items-center space-x-1 sm:space-x-2 md:space-x-3 opacity-60 cursor-not-allowed">
                  ...
                  <div class="text-left">
                    <div class="text-[12px] sm:text-xs text-gray-300">Coming soon on</div>
                    <div class="text-[12px] sm:text-xs md:text-sm font-semibold text-white">App Store</div>
                  </div>
                </div>
                <div class="bg-gray-600 rounded-lg px-1.5 sm:px-3 md:px-4 py-1 sm:py-2 md:py-3 flex items-center space-x-1 sm:space-x-2 md:space-x-3 opacity-60 cursor-not-allowed">
                  <svg class="w-3 h-3 sm:w-6 sm:h-6 md:w-8 md:h-8 text-white" fill="currentColor" viewBox="0 0 24 24">
                  ...
                  <div class="text-left">
                    <div class="text-[12px] sm:text-xs text-gray-300">Coming soon on</div>
                    <div class="text-[12px] sm:text-xs md:text-sm font-semibold text-white">Google Play</div>
                  </div>
                </div>
              </div>
            </div>
