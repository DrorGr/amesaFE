...
export class TranslationService {
  ...
  private loadTranslations(language: Language): void {
    ...
    this.http.get<{success: boolean, data: TranslationsResponse}>(`${environment.apiUrl}/api/v1/translations/${language}`)
      .pipe(
  ...
  getTranslations(language: Language): Observable<Translations> {
    ...
    return this.http.get<{success: boolean, data: TranslationsResponse}>(`${environment.apiUrl}/api/v1/translations/${language}`)
      .pipe(
  ...
  getAvailableLanguagesFromBackend(): Observable<LanguageInfo[]> {
    return this.http.get<{success: boolean, data: LanguageInfo[]}>(`${environment.apiUrl}/api/v1/translations/languages`)
      .pipe(
        map(response => response.data),
        tap(languages => {
