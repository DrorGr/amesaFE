  ...
  template: `
    <div class="relative">
      <button
        ...
        <span class="text-black dark:text-white font-bold">{{ getCurrentLanguageCode() }}</span>
        <svg class="w-3 h-3 md:w-4 md:h-4 transition-transform duration-200" [class.rotate-180]="isDropdownOpen" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
        </svg>
      </button>

      @if (isDropdownOpen) {
        <div class="absolute right-0 mt-2 w-40 bg-white dark:bg-gray-800 rounded-lg shadow-lg border border-gray-200 dark:border-gray-700 py-2 z-50">
