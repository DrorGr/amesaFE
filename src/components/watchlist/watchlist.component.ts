  ...
  template: `
    <div class="min-h-screen bg-gradient-to-br from-gray-50 to-blue-50 dark:from-gray-900 dark:to-gray-800 p-4 md:p-8">
      <div class="max-w-6xl mx-auto">
        ...
        <!-- Watchlist Grid -->
        <ng-container *ngIf="watchlistItems().length > 0; else noWatchlist">
            ...
            <div 
              *ngFor="let item of watchlistItems()" 
              ...
              <div class="relative h-48 bg-gray-200">
                <img 
                  [src]="item.house.images?.[0]?.imageUrl || ''" 
                ...
                <button
                  ...
                  [class.animate-pulse]="isRemoving()(item.houseId)"
                ...
                <button
                  ...
                  class="absolute top-4 left-4 bg-white/90 dark:bg-gray-800/90 hover:bg-white dark:hover:bg-gray-800 p-2 rounded-full shadow-lg transition-all duration-300 cursor-pointer z-10"
              ...
              <div class="p-6">
                ...
                <div *ngIf="item.house.maxParticipants" class="mb-4 p-3 bg-gray-100 dark:bg-gray-700 rounded-lg">
                  <div class="text-sm text-gray-600 dark:text-gray-400 mb-1">
                    {{ translate('participants.count', { count: item.house.uniqueParticipants }) }}
                  </div>
                  <div class="text-xs text-gray-500 dark:text-gray-500">
                    {{ translate('participants.maxParticipants', { max: item.house.maxParticipants }) }}
                  </div>
                  ...
                  <div *ngIf="item.house.remainingParticipantSlots !== undefined && item.house.remainingParticipantSlots > 0" class="text-xs text-green-600 dark:text-green-400 mt-1">
                    {{ translate('participants.remainingSlots', { count: item.house.remainingParticipantSlots }) }}
                  </div>
                </div>
                ...
                <button
                  ...
                  class="w-full bg-blue-600 hover:bg-blue-700 dark:bg-blue-700 dark:hover:bg-blue-600 text-white py-3 px-4 rounded-lg font-semibold transition-all duration-200">
  ...
  loadWatchlist(): void {
    this.watchlistService.getWatchlist().subscribe({
      ...
      error: (error) => {
        ...
        this.toastService.showError(this.translate('watchlist.loadError'));
      }
    });
  }
  ...
  removeFromWatchlist(event: Event, houseId: string): void {
    ...
    this.watchlistService.removeFromWatchlist(houseId).subscribe({
      next: () => {
        ...
        this.watchlistItems.set(items);
        this.toastService.showSuccess(this.translate('watchlist.removeSuccess'));
        current.delete(houseId);
        this.isRemoving.set(new Map(current));
      },
      error: (error) => {
        console.error('Error removing from watchlist:', error);
        this.toastService.showError(this.translate('watchlist.removeError'));
        current.delete(houseId);
        this.isRemoving.set(new Map(current));
      }
    });
  ...
  toggleNotification(event: Event, item: WatchlistItem): void {
    ...
    this.watchlistService.toggleNotification(item.houseId, newEnabled).subscribe({
      ...
      error: (error) => {
        ...
        this.toastService.showError(this.translate('watchlist.notificationToggleError'));
      }
    });
  }
